...
pub async fn osu(
    ...
    let title = format!("<:osu:1485724464686825513> osu! user: {}", username);
    ...
    let online_str = if is_online {
        "<a:online:1485723774375690511> Online".to_string()
    } else {
        match response["last_visit"].as_str() {
            Some(last_visit) => match chrono::DateTime::parse_from_rfc3339(last_visit) {
                Ok(dt) => format!(
                    "<a:offline:1485723772223885513> Offline • last seen <t:{}:R>",
                    dt.timestamp()
                ),
                Err(_) => "<a:offline:1485723772223885513> Offline".to_string(),
            },
            None => "<a:offline:1485723772223885513> Offline".to_string(),
        }
    };
...
pub async fn osur(
    ...
    fn grade_2_emoji(rank: &str) -> String {
        let e_id = match rank {
            "A" => "1485722721848528977",
            "B" => "1485722722779791521",
            "C" => "1485722723672916038",
            "D" => "1485722724671291544",
            "F" => "1485722251818045591",
            "S" => "1485722726277841016",
            "SH" => "1485722727598784686",
            "X" => "1485722728282456134",
            "XH" => "1485722729830154383",
    ...
    let title = format!("<:osu:1485724464686825513> {}'s recent score", username);
    ...
    let embed = CreateEmbed::new()
        ...
        .field("Stars", format!("{:.2} <:star:1485726264613539931>", stars), true)
        .field("Mods", mods_str, true)
        .field(
            "Hits",
            format!(
                "{} <:hit300:1485722895790374912> • {} <:hit100:1485722899892666553> • {} <:hit50:1485722888429637773> • {} <:hit0:1485722902736142618>",
...
pub async fn lb(ctx: Context<'_>) -> Result<(), Error> {
    ...
    let mut embed = CreateEmbed::new()
        .title("<:osu:1485724464686825513> osu! global leaderboard")
...
pub async fn hgraph(
    ...
    let c_emojis = [
        "<:circlePink:1485722183689961544>",
        "<:circleBlue:1485722179990454552>",
        "<:circleGreen:1485722178908455004>",
        "<:circleOrange:1485722178115604602>",
        "<:circlePurple:1485722177129943062>",
    ];
    ...
    let description = sorted_users
        ...
        .map(|(_i, u)| {
            ...
            let change = if diff > 0 {
                format!("<:up:1485722199993225406> {}", format_num(diff as u64))
            } else if diff < 0 {
                format!(
                    "<:down:1485722198550249492> {}",
                    format_num(diff.unsigned_abs())
                )
            } else {
                "<:none:1485722176144539892> No change".to_string()
            };
